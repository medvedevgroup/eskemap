...
parser = args.ArgumentParser(description="This script extracts the i-th entry (starting from 0) from a multiple FASTA file.")
...
entries = [r for r in SeqIO.parse(open(arguments.s, 'r'), "fasta")]

if arguments.i > len(entries):
	print("ERROR: i-th entry does not exist in input file!", file=stderr)
	exit(-1)

SeqIO.write(entries[arguments.i], open(arguments.o, 'w'), "fasta")
